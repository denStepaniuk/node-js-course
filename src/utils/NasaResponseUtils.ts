  ...
  transformMeteorResponse(response: any): NearEarthObjectsResponse {
    ...
    for (const key in response.data.near_earth_objects) {
      if (response.data.near_earth_objects.hasOwnProperty(key)) {
        responseBody[key] = response.data.near_earth_objects[key].map(function (meteor: any) {
          return {
            id: meteor.id,
            name: meteor.name,
...
function estimatedDiameterInMeters(estimatedDiameterMin: number, estimatedDiameterMax: number): number {
